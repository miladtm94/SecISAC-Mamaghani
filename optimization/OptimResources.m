function [Sol, slackvars] = OptimResources(params, channels, slackvars, flag, CNR)
...
%         
     ...
     minimize(trace(SumVk) + trace(W) + sum(P)) % Minimize total power
...
Sol.V_opt = V; % Optimized downlink user precoding covariance matrices
Sol.W_opt = W; % Optimized radar sensing covariance matrix
Sol.P_opt = P; % Optimized power allocation
Sol.OF = cvx_optval;
